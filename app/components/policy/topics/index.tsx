...
// Derive a stable slug from the topic title for use as DOM id
...
export default function Topics({ topics: propTopics }: TopicsProps) {
  ...
  useEffect(() => {
    ...
    topics.forEach((topic) => {
      const slug = toSlug(topic.title);
      const el = document.getElementById(`policy-topic-${slug}`);
      if (!el) return;
  ...
  const handleTopicChange = (slug: string) => {
    ...
    document.getElementById(`policy-topic-${slug}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_3fr] lg:gap-7">
          ...
          <div className="space-y-5">
            {topics.map((topic, index) => {
              const slug = toSlug(topic.title);
              return (
                <div key={topic._id} id={`policy-topic-${slug}`} className="space-y-3 scroll-mt-6">
