...
import { faqTopics } from '../../../lib/data/faq-page-topics';
...
export default function TopicsAccordion() {
  const [activeTopicId, setActiveTopicId] = useState(faqTopics[0]?.id ?? '');
  const [openIndexesByTopic, setOpenIndexesByTopic] = useState<Record<string, number>>(
    () =>
      faqTopics.reduce<Record<string, number>>((acc, topic) => {
        acc[topic.id] = 0;
        return acc;
      }, {}),
  );
          ...
          <div className="space-y-5">
            {faqTopics.map((topic , index) => (
              <div key={topic.id} id={`faq-topic-${topic.id}`} className="space-y-3">
                ...
                <p className={`mb-4 mt-2 text-sm sm:text-base ${palette.muted}`}>{topic.description}</p>

                <ul className="space-y-4">
                  {topic.items.map((item, index) => {
                    const open = openIndexesByTopic[topic.id] === index;
                    const panelId = `faq-topic-panel-${topic.id}-${index}`;
                    const buttonId = `faq-topic-trigger-${topic.id}-${index}`;

                    return (
                      <li
                        key={item.id}
                        className={`group relative overflow-clip rounded-2xl border border-[#282828] backdrop-blur-xl transition-all duration-500 hover:-translate-y-0.5 ${palette.border} ${palette.panel} ${palette.shadow} ${open ? 'border-[#0183C1]' : ''
                          }`}
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            setOpenIndexesByTopic((prev) => ({
                              ...prev,
                              [topic.id]: prev[topic.id] === index ? -1 : index,
                            }))
                          }
                        ...
                        >
                          ...
                          >
                            <svg className={`relative transition-transform duration-500 ${palette.icon} ${open ? 'rotate-45 text-[#0183C1]' : ''}`} xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="0 0 34 34" fill="none">
                          ...
                          <div className="flex flex-1 flex-col gap-2">
                            ...
                            <div
                              ...
                              className={`overflow-clip text-sm leading-relaxed transition-[max-height] duration-500 ease-out ${palette.muted} ${open ? 'max-h-40' : 'max-h-0'
                                }`}
                            >
